...
class App extends Component {
        ...
        async function () {
          ...
          if (this.state.scores.length < 20 || this.isHighScore(guessesCopy)) {
            if (this.state.user) {
              ...
              alert('You need to login to save the highscore!');
  ...
  handleSignupOrLogin = async () => {
    ...
    // BELOW CODE IS FOR IN CLASS EXAMPLE - purely proof of concept
    // THERE ARE BETTER WAYS TO DO THIS, but we used the functions we already had available...

    // if (this.getWinTries()) {
    //   if (this.state.scores.length < 20 || this.isHighScore(this.state.guesses)) {
    ...
    return (
      <div>
        <header className="header-footer">
          R E A C T &nbsp;&nbsp;&nbsp; M A S T E R M I N D {process.env.REACT_APP_X}
        </header>
        <Switch>
          <Route
            exact
